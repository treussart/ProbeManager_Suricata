    ...
    def test_signature(self):
        ...
        with open(settings.BASE_DIR + '/suricata/tests/data/test.pcap', 'rb') as f:
            response = self.client.post('/admin/suricata/signaturesuricata/add/',
                                        {'rev': '1',
                                         'rule_full': 'alert ip 192.168.0.66 any -> 104.199.65.12 any (msg:"Test Pcap '
                                                      'match"; threshold: type limit, track by_src, seconds 3600, '
                                                      'count 1; classtype:misc-attack; sid:667; rev:1;)',
                                         'sid': '667',
                                         'classtype': '29',
                                         'msg': 'Test Pcap match',
                                         'file_test_success': f,
        ...
        with open(settings.BASE_DIR + '/suricata/tests/data/test.pcap', 'rb') as f:
            response = self.client.post('/admin/suricata/signaturesuricata/add/',
                                        {'rev': '1',
                                         'rule_full': 'alert ip 192.168.10.66 any -> 104.199.5.12 any (msg:"Test Pcap '
                                                      'match"; threshold: type limit, track by_src, seconds 3600, '
                                                      'count 1; classtype:misc-attack; sid:667; rev:1;)',
                                         'sid': '668',
                                         'classtype': '29',
